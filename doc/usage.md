...
The values of certain dimension or attribute components within a data cube may be restricted to some enumerated set of values. For such
components, a _code list_ should be defined containing the permissible values. In line with the recommendations within the
[RDF data cube specification](https://www.w3.org/TR/vocab-data-cube/), `table2qb` generates a `skos:ConceptScheme` from a CSV
file defining the values within the list. The definition CSV file should contain the following columns:
...
* `Sort Priority`: Optional numeric value indicating the position of the value within the code list. Some user interfaces may use this value
  to sort the code list values for dispaly purposes.
  
The [employment example](../examples/employment/README.md) contains a [gender codelist](../examples/employment/csv/gender.csv). Note the optional `Description` and `Sort Priority` columns
...
the one generated by `codelist-pipeline`.
...
column in the data and how to process the contained data. This configuration file should contain the following columns:
...
* `component_attachment`: Either blank, or one of `qb:dimension`, `qb:measure` or `qb:attribute` to indicate whether the column defines a dimension, attribute or measure of the observations. If blank, the column contains observation values (see [Measure dimension cubes](#measure-dimension-cubes) for more details).
...
* `value_transformation`: Optional transformation to apply to cell values in the corresponding column. If specified it should be either [slugize](#slugize) or [unitize](#unitize).

An example column configuration file is defined in the [employment example](../examples/employment/columns.csv). The columns must define all columns within an observations file, but can also contain definitions
for additional columns that do not exist. This means that all known columns for multiple different cubes can be defined within a single definition file (subject to the constraints that the values within the `title`
and `name` columns must be unique).
...
and not its `title` (i.e. `gender` insteaad of `Gender`).
